...
@testset "from_dict" begin
    @testset "test maybe union type" begin
        d = Dict{String, Any}(
            ...
            "union_of_alias" => Dict{String, Any}(
                "option_b" => Dict{String, Any}(
                    "x" => 1
                )
            ),
            "union_of_reflects" => Dict{String, Any}(
                "type" => "Main.TestFromDict.OptionReflectC",
                "y" => 1
            ),
            "maybe_of_alias" => nothing,
            "maybe_of_reflects" => nothing,
            "totally_mixed" => Dict{String, Any}(
                "type" => "Main.TestFromDict.OptionReflectB",
                "x" => 1
            ),
            "maybe_totally_mixed" => Dict{String, Any}(
    ...
    @testset "test dynamic union type" for option in [
        ...
        for idx in 1:fieldcount(TestUnionType)
            ...
            @test from_dict_union_type_dynamic(
                TestUnionType,
                OptionField(field),
                type, d[string(field)]) == getfield(option, field)
        end        
    end
end
